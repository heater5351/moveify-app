...
import { Plus, Minus, Check, Loader2, X, AlertCircle } from 'lucide-react';
import {
    ...
    <div className="flex flex-col min-h-0 h-full overflow-y-auto">
      ...
      {!readOnly && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 mb-3 shrink-0">
            {catalog.map(a => (
              <button
                key={a.key}
          ...
          {selected && (
            <div className="rounded-lg bg-gray-50 border border-gray-200 p-3 shrink-0">
          ...
          {!selected && (
            <p className="text-xs text-gray-400 shrink-0">Pick an assessment above to record values.</p>
          )}
        </>
      )}
    </div>
