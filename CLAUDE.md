...
| `scribe_session_outcomes` | session_id, patient_id, prom_key, responses_enc (AES-256-GCM), score, score_band, cliniko_attachment_id, completed_at | Patient-completed outcome measures (PROMs — Phase 4). Raw responses **encrypted** (sensitive self-report); score+band plain for the note/trend. `UNIQUE(session_id, prom_key)`. Scored server-side by `services/prom-scoring.js`; catalog `data/prom-catalog.json`. `cliniko_attachment_id` reserved for Phase 4b (PDF→Cliniko, not yet wired) |
