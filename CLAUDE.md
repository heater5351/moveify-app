...
|-------|-------------|-------|
| `users` | id, email, password_hash, role (`'clinician'`/`'patient'`), name, dob, phone, condition, is_admin, login_username | Single table for both roles. `is_admin` controls admin privileges for clinicians. **`email` is NOT unique** (households share one) — identity is `firebase_uid`. `login_username` (nullable, partial-unique) is the alternate login for a second patient on a shared email; see "Shared-email login" |
