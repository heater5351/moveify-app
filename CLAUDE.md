...
| `scribe_session_measurements` | session_id, assessment_key, side (`left`/`right`/`bilateral`), measure_key, value (NUMERIC), value2 (NUMERIC, compound only e.g. BP diastolic), unit | Tap-captured in-session assessments (scribe Phase 3). **Plain numerics** (not encrypted — kept queryable, same stance as completions/check-ins). `UNIQUE(session_id, assessment_key, side, measure_key)` for upsert. `measure_key` matches a `normative-data.json` key → graded into the SOAP `OBJECTIVE MEASUREMENTS` block via `services/measurement-render.js`. Catalog: `data/assessment-catalog.json` (30 assessments; measure `input` ∈ presets/keypad/compound/toggle, optional per-measure `laterality`) |
