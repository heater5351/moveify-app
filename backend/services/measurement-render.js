 ...
 *
 * Returns an array of strings (one per side/measure, plus an asymmetry line), the
 * same shape the SOAP prompt-assembly layer already consumes for program diffs.
...
/**
 * @param {Array<{assessment_key, measure_key, side, value, unit}>} rows
...
function renderMeasurements(rows, age, sex) {
  ...
  // Group by measure (assessment + measure key), preserving first-seen order so
  // the note lists them in the order they were captured.
  const order = [];
  ...
  for (const key of order) {
    ...
    // Side-to-side asymmetry (>=10%) — a key clinical flag for bilateral tests.
